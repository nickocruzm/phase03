...
declaration: identifiers COLON INTEGER SEMICOLON
           ...
           | identifiers COLON ENUM L_PAREN enum_list R_PAREN
...
statement
    : var ASSIGN expression {
        if (!$1) {
            printf( "NULL value in assignment at line %d\n", yylineno);
            exit(1);
        }
        printf(". %s\n", $1);
    }
    ...
    | READ vars{
        if (!$2) {
            fprintf(stderr, "NULL value in READ at line %d\n", yylineno);
            exit(1);
        }
    ...
    | WRITE vars{
        if (!$2) {
            fprintf(stderr, "NULL value in WRITE at line %d\n", yylineno);
            exit(1);
        }
        printf("> %s\n", $2);
    }
...
relation_expr
    : expression comp expression {
        ...
        if ($1 && $3) {
            ...
            fprintf(stderr, "NULL value in assignment at line %d: $1=%p, $3=%p\n", yylineno, $1, $3);
            exit(1);
        }
    }
